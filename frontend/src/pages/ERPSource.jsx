...
const DIM_SUGGESTIONS_BY_ERP = {
  // ── SAP S/4HANA ─────────────────────────────────────────────────────────────
  // Sources: SAP S/4HANA Finance Data Model (SAP Help Portal), SAP SD/MM/HCM table documentation,
  // Kimball Group Dimensional Modeling Techniques (Ralph Kimball), SAP Note 2324615 (ACDOCA).
  sap_s4hana: {
    finance: [
      // S/4HANA Universal Journal — ACDOCA replaces classic BKPF+BSEG split
      { sourceTable: "ACDOCA", additionalSources: ["ACDOCP"],           relationship: "N:1", targetName: "fact_gl_transactions",    type: "fact", businessName: "GL Transactions (S/4HANA Universal Journal)", description: "S/4HANA Universal Journal: ACDOCA (actual line items) merged with ACDOCP (plan). Replaces BKPF+BSEG. Grain: one row per account assignment line. Key: MANDT+RLDNR+RBUKRS+GJAHR+BELNR+DOCLN" },
      // Classic ECC fallback (pre-S/4HANA migration or parallel ledger)
      { sourceTable: "BSEG",   additionalSources: ["BKPF"],             relationship: "N:1", targetName: "fact_gl_transactions_classic", type: "fact", businessName: "GL Transactions (Classic ECC)",    description: "Classic: BSEG (line items) merged with BKPF (document header) via MANDT+BUKRS+BELNR+GJAHR. Use for ECC systems or during S/4HANA migration period." },
      { sourceTable: "SKAT",   additionalSources: ["SKA1"],              relationship: "N:1", targetName: "dim_account",             type: "dim",  businessName: "GL Accounts",              description: "G/L account descriptions (SKAT) merged with chart of accounts master (SKA1) via KTOPL+SAKNR" },
      { sourceTable: "T009B",  relationship: "1:1",                                           targetName: "dim_period",              type: "dim",  businessName: "Fiscal Periods",           description: "SAP fiscal year variant periods (T009B) — period number, start/end date, posting control" },
      { sourceTable: "TCURR",  relationship: "1:1",                                           targetName: "dim_currency_rate",       type: "dim",  businessName: "Currency Rates",           description: "Exchange rates by currency pair and validity date (TCURR)" },
      { sourceTable: "BSID",   additionalSources: ["BSAD"],              relationship: "N:1", targetName: "fact_ar_line_items",      type: "fact", businessName: "AR Line Items (Open+Cleared)", description: "AR open items (BSID) UNION with cleared items (BSAD) for full AR aging. Grain: one row per customer/document/line." },
      { sourceTable: "BSIK",   additionalSources: ["BSAK"],              relationship: "N:1", targetName: "fact_ap_line_items",      type: "fact", businessName: "AP Line Items (Open+Cleared)", description: "AP open (BSIK) UNION cleared (BSAK) for full payable aging and DPO analysis." },
    ],
    order_management: [
      // SD: Sales Document Items (VBAP) + Header (VBAK) — N:1 JOIN
      { sourceTable: "VBAP",   additionalSources: ["VBAK"],              relationship: "N:1", targetName: "fact_order_lines",        type: "fact", businessName: "Sales Order Lines",        description: "VBAP (SD item data: qty, price, material, schedule lines) joined with VBAK (header: sold-to, doc type) via VBELN. Kimball grain: one row per order line." },
      // VBAK 1:N — order header spawns partner functions + pricing conditions
      { sourceTable: "VBAK",   relationship: "1:N", additionalTargets: ["fact_sales_order_partners","fact_sales_order_conditions"], targetName: "fact_sales_orders", type: "fact", businessName: "Sales Orders (Header)", description: "VBAK order header → fact_sales_orders (header KPIs) + fact_sales_order_partners (VBPA: sold-to/ship-to/bill-to) + fact_sales_order_conditions (KONV: pricing)" },
      // SD: Delivery — LIKP (header) + LIPS (items) — N:1 JOIN for shipment fact
      { sourceTable: "LIPS",   additionalSources: ["LIKP"],              relationship: "N:1", targetName: "fact_shipments",          type: "fact", businessName: "Outbound Deliveries / Shipments", description: "LIPS (delivery items: material, qty, batch) merged with LIKP (delivery header: ship-to, actual GI date) via VBELN. Grain: one delivery line." },
      // SD: Billing — VBRP (items) + VBRK (header) — N:1 JOIN for AR invoice fact
      { sourceTable: "VBRP",   additionalSources: ["VBRK"],              relationship: "N:1", targetName: "fact_ar_invoices",        type: "fact", businessName: "Billing Documents / AR Invoices", description: "VBRP (billing items: net value, tax, material) merged with VBRK (billing header: payer, invoice date, payment terms) via VBELN. Standard Kimball revenue fact." },
      { sourceTable: "KNA1",   additionalSources: ["KNVV","KNVP"],       relationship: "N:1", targetName: "dim_customer",            type: "dim",  businessName: "Customers",                description: "General customer data (KNA1) merged with sales area data (KNVV: sales org/channel/division) and partner functions (KNVP) via KUNNR" },
      { sourceTable: "MARA",   additionalSources: ["MARC","MAKT"],        relationship: "N:1", targetName: "dim_product",             type: "dim",  businessName: "Products / Materials",     description: "Material master (MARA: type, base UoM, division) + plant extension (MARC: MRP, lead time) + multilingual description (MAKT) via MATNR" },
      { sourceTable: "TVKOT",  relationship: "1:1",                                           targetName: "dim_sales_org",           type: "dim",  businessName: "Sales Organizations",      description: "Sales organization master (TVKOT) — company code assignment, region, division hierarchy" },
    ],
    accounts_payable: [
      // LIV: Logistics Invoice Verification — RBKP (header) + RSEG (items) N:1
      { sourceTable: "RSEG",   additionalSources: ["RBKP"],              relationship: "N:1", targetName: "fact_ap_invoices",        type: "fact", businessName: "AP Invoice Documents (LIV)", description: "RSEG (invoice line items: material, qty, amount, PO ref) merged with RBKP (invoice header: vendor, doc date, payment terms) via BELNR+GJAHR. Kimball grain: one invoice line." },
      { sourceTable: "RBKP",   relationship: "1:N", additionalTargets: ["fact_ap_invoice_holds"], targetName: "fact_ap_invoice_headers", type: "fact", businessName: "AP Invoice Headers", description: "RBKP header → fact_ap_invoice_headers (header KPIs) + fact_ap_invoice_holds (RBKPH: blocking reasons for exception reporting)" },
      { sourceTable: "PAYR",   relationship: "1:1",                                           targetName: "fact_ap_payments",        type: "fact", businessName: "AP Payments",              description: "Payment medium data (PAYR) — check number, ACH/wire reference, payment date, bank clearing" },
      { sourceTable: "LFA1",   additionalSources: ["LFB1","LFM1"],       relationship: "N:1", targetName: "dim_supplier",            type: "dim",  businessName: "Suppliers / Vendors",      description: "Vendor general data (LFA1) + company-code extension (LFB1: payment method, reconciliation acct) + purchasing org (LFM1: incoterms, payment terms) via LIFNR" },
      { sourceTable: "T052",   relationship: "1:1",                                           targetName: "dim_payment_terms",       type: "dim",  businessName: "Payment Terms",            description: "SAP payment term keys (T052) — due date rules, cash discount percentages and days" },
    ],
    procurement: [
      // MM: Purchase Order — EKPO (items) + EKKO (header) N:1
      { sourceTable: "EKPO",   additionalSources: ["EKKO"],              relationship: "N:1", targetName: "fact_po_lines",           type: "fact", businessName: "PO Line Items",            description: "EKPO (PO items: material, qty, net price, plant, delivery date) merged with EKKO (PO header: vendor, purchasing org, doc type) via EBELN. Grain: one PO line." },
      { sourceTable: "EKKO",   relationship: "1:N", additionalTargets: ["fact_po_account_assignments","fact_po_conditions"], targetName: "fact_purchase_orders", type: "fact", businessName: "Purchase Orders (Header)", description: "EKKO → fact_purchase_orders + account assignments (EKKN: cost center/WBS) + pricing conditions (EKPO/KONV)" },
      // S/4HANA: MATDOC replaces classic MKPF+MSEG (Material Document universal journal)
      { sourceTable: "MATDOC", relationship: "1:1",                                           targetName: "fact_goods_receipts",     type: "fact", businessName: "Goods Receipts / GI (S/4HANA MATDOC)", description: "S/4HANA Material Document universal table (MATDOC) replaces MKPF+MSEG split. Grain: one material movement line. Filter BWART IN ('101','102') for GR against PO." },
      // Classic ECC fallback
      { sourceTable: "MSEG",   additionalSources: ["MKPF"],              relationship: "N:1", targetName: "fact_goods_receipts_classic", type: "fact", businessName: "Goods Receipts (Classic MKPF+MSEG)", description: "Classic: MSEG (material document items) merged with MKPF (header) via MBLNR+MJAHR. Use for ECC or during S/4HANA migration." },
      { sourceTable: "LFA1",   additionalSources: ["LFB1","LFM1"],       relationship: "N:1", targetName: "dim_supplier",            type: "dim",  businessName: "Suppliers / Vendors",      description: "Vendor master: general (LFA1) + company-code (LFB1) + purchasing org (LFM1)" },
      { sourceTable: "MARA",   additionalSources: ["MARC"],              relationship: "N:1", targetName: "dim_item",                type: "dim",  businessName: "Materials / Items",        description: "Material master (MARA) merged with plant-level data (MARC: MRP type, procurement type, lead time)" },
    ],
    inventory: [
      // S/4HANA: MATDOC is the single source of truth for all material movements
      { sourceTable: "MATDOC", relationship: "1:1",                                           targetName: "fact_inventory_movements", type: "fact", businessName: "Inventory Movements (S/4HANA MATDOC)", description: "S/4HANA universal material document (MATDOC) — all goods movements: receipts, issues, transfers, adjustments. Grain: one movement line. Key: MBLNR+MJAHR+ZEILE." },
      // Classic ECC fallback
      { sourceTable: "MSEG",   additionalSources: ["MKPF"],              relationship: "N:1", targetName: "fact_inventory_movements_classic", type: "fact", businessName: "Inventory Movements (Classic MSEG+MKPF)", description: "Classic material document: MSEG items + MKPF header via MBLNR+MJAHR. ECC/migration fallback." },
      { sourceTable: "MARD",   relationship: "1:1",                                           targetName: "fact_inventory_snapshot", type: "fact", businessName: "Stock Snapshot (by Plant/SLoc)", description: "MARD — unrestricted, in-QI, blocked, transit stock by material/plant/storage location. Kimball periodic snapshot fact." },
      { sourceTable: "MARA",   additionalSources: ["MARC","MAKT","MBEW"], relationship: "N:1", targetName: "dim_item",              type: "dim",  businessName: "Materials",                description: "Material master (MARA) + plant extension (MARC) + multilingual description (MAKT) + valuation class (MBEW) via MATNR" },
      { sourceTable: "T001W",  relationship: "1:1",                                           targetName: "dim_plant",               type: "dim",  businessName: "Plants / Warehouses",      description: "Plant master (T001W) — distribution center, factory, country, company code assignment" },
    ],
    human_resources: [
      { sourceTable: "PA0001", additionalSources: ["PA0002","PA0006","PA0007"], relationship: "N:1", targetName: "dim_employee", type: "dim",  businessName: "Employees",                description: "Infotype 0001 (org assignment: cost center, position) + 0002 (personal data) + 0006 (address) + 0007 (planned work time) via PERNR. Kimball conformed employee dimension." },
      { sourceTable: "PA0001", relationship: "1:N", additionalTargets: ["fact_employee_compensation","fact_employee_actions"], targetName: "fact_assignments", type: "fact", businessName: "Employee Assignments", description: "PA0001 org data → fact_assignments + PA0008 (compensation: salary, grade) + PA0000 (HR actions: hire, transfer, terminate)" },
      { sourceTable: "PA2001", relationship: "1:1",                                           targetName: "fact_absence",            type: "fact", businessName: "Absences",                 description: "PA2001 absence/attendance records — absence type, calendar hours, cost center, quota deduction" },
      { sourceTable: "HRP1000",additionalSources: ["HRP1001"],           relationship: "N:1", targetName: "dim_department",          type: "dim",  businessName: "Org Units / Cost Centers", description: "HRP1000 (org objects: org unit, position, job) + HRP1001 (relationships: A/B 002 hierarchy) via OBJID. Represents org hierarchy for Kimball drill-down." },
    ],
  },

  // ── Microsoft Dynamics 365 Finance & Operations ──────────────────────────────
  // Sources: D365 F&O Data Entity Catalog (learn.microsoft.com), AOT table browser,
  // Microsoft FastTrack dimensional modeling guidance, Kimball Group.
  dynamics_365_fo: {
    finance: [
      // Correct entity name per D365 F&O AOT: GeneralJournalAccountEntry (line) + GeneralJournalEntry (header)
      // Note: some docs reference "GeneralJournalLine" — the canonical AOT entity is GeneralJournalAccountEntry
      { sourceTable: "GeneralJournalAccountEntry", additionalSources: ["GeneralJournalEntry"], relationship: "N:1", targetName: "fact_gl_transactions", type: "fact", businessName: "GL Transactions", description: "GeneralJournalAccountEntry (account line: MainAccount, amount, financial dimension) merged with GeneralJournalEntry (header: journal batch, ledger, posting date) via GeneralJournalEntry RecId. Grain: one account line per journal entry." },
      { sourceTable: "MainAccount",              relationship: "1:1",                                           targetName: "dim_account",             type: "dim",  businessName: "Chart of Accounts",        description: "MainAccount entity — account number, account type (Balance Sheet/P&L), category, consolidation account" },
      { sourceTable: "FiscalCalendarPeriod",     additionalSources: ["FiscalCalendarYear"],    relationship: "N:1", targetName: "dim_period",            type: "dim",  businessName: "Fiscal Periods",           description: "FiscalCalendarPeriod (period name, start/end) merged with FiscalCalendarYear via FiscalCalendar RecId" },
      // AR: CustInvoiceTrans (lines) + CustInvoiceJour (header) for invoice fact
      { sourceTable: "CustInvoiceTrans",         additionalSources: ["CustInvoiceJour"],       relationship: "N:1", targetName: "fact_ar_invoices",      type: "fact", businessName: "AR Invoice Lines",         description: "CustInvoiceTrans (invoice line: item, qty, amount, tax) merged with CustInvoiceJour (header: customer, invoice date, payment terms) via InvoiceId. Standard Kimball revenue/AR fact." },
      { sourceTable: "CustTrans",                additionalSources: ["CustTransOpen"],         relationship: "N:1", targetName: "fact_ar_transactions",   type: "fact", businessName: "AR Transactions (Open+History)", description: "CustTrans (all settled and open AR postings) merged with CustTransOpen (outstanding balances) for DSO/aging analysis" },
      // AP: VendInvoiceTrans (lines) + VendInvoiceJour (header) for invoice fact
      { sourceTable: "VendInvoiceTrans",         additionalSources: ["VendInvoiceJour"],       relationship: "N:1", targetName: "fact_ap_invoices",      type: "fact", businessName: "AP Invoice Lines",         description: "VendInvoiceTrans (invoice line: item, qty, amount, purchase category) merged with VendInvoiceJour (header: vendor, invoice date, approval status) via InvoiceId." },
      { sourceTable: "VendTrans",                additionalSources: ["VendTransOpen"],         relationship: "N:1", targetName: "fact_ap_transactions",   type: "fact", businessName: "AP Transactions (Open+History)", description: "VendTrans (all AP postings) merged with VendTransOpen (outstanding) for DPO/aging analysis" },
    ],
    order_management: [
      { sourceTable: "SalesLine",                additionalSources: ["SalesTable"],            relationship: "N:1", targetName: "fact_order_lines",       type: "fact", businessName: "Sales Order Lines",        description: "SalesLine (item, qty, price, delivery date) merged with SalesTable (header: customer, order type, sales pool) via SalesId. Kimball grain: one order line." },
      { sourceTable: "SalesTable",               relationship: "1:N", additionalTargets: ["fact_sales_order_charges","fact_sales_confirmations"], targetName: "fact_sales_orders", type: "fact", businessName: "Sales Orders (Header)", description: "SalesTable → fact_sales_orders (header KPIs) + MarkupTrans (misc charges) + CustConfirmTrans (order confirmations)" },
      { sourceTable: "CustTable",                additionalSources: ["DirPartyTable","LogisticsPostalAddress"], relationship: "N:1", targetName: "dim_customer", type: "dim", businessName: "Customers",           description: "CustTable (customer account, currency, credit limit) merged with DirPartyTable (legal name, party type) and LogisticsPostalAddress (billing address) via AccountNum/Party" },
      { sourceTable: "EcoResProduct",            additionalSources: ["EcoResProductTranslation","InventTable"], relationship: "N:1", targetName: "dim_product", type: "dim", businessName: "Products",             description: "EcoResProduct (product number, type, subtype) merged with EcoResProductTranslation (multilingual name) and InventTable (item model group, item group) via RecId/ItemId" },
    ],
    accounts_payable: [
      { sourceTable: "VendInvoiceTrans",         additionalSources: ["VendInvoiceInfoTable"],  relationship: "N:1", targetName: "fact_ap_invoice_lines",  type: "fact", businessName: "AP Invoice Lines (Pending)", description: "VendInvoiceTrans (pending invoice lines) merged with VendInvoiceInfoTable (header: vendor, invoice number, match status) for 3-way match analytics" },
      { sourceTable: "VendTable",                additionalSources: ["DirPartyTable","LogisticsPostalAddress"], relationship: "N:1", targetName: "dim_supplier", type: "dim", businessName: "Suppliers / Vendors",  description: "VendTable (vendor account, currency, payment terms) + DirPartyTable (legal entity name) + LogisticsPostalAddress (remittance address)" },
    ],
    procurement: [
      { sourceTable: "PurchLine",                additionalSources: ["PurchTable"],            relationship: "N:1", targetName: "fact_po_lines",           type: "fact", businessName: "PO Lines",                 description: "PurchLine (item, qty, net price, procurement category, deliver date) merged with PurchTable (header: vendor, purchasing policy, status) via PurchId" },
      { sourceTable: "PurchTable",               relationship: "1:N", additionalTargets: ["fact_po_charges","fact_po_confirmations"], targetName: "fact_purchase_orders", type: "fact", businessName: "Purchase Orders (Header)", description: "PurchTable → fact_purchase_orders + MarkupTrans (charges) + VendConfirmTrans (order confirmations / acknowledgements)" },
      // InventTrans: N:1 with InventDim (dimension ref is mandatory for all inventory transactions)
      { sourceTable: "InventTrans",              additionalSources: ["InventDim"],             relationship: "N:1", targetName: "fact_goods_receipts",     type: "fact", businessName: "Goods Receipts / Issues",  description: "InventTrans (all inventory transactions: receipt, issue, transfer) merged with InventDim (dimension: site, warehouse, serial, batch, location) via InventDimId. Filter StatusReceipt for GR." },
      { sourceTable: "VendTable",                additionalSources: ["DirPartyTable"],         relationship: "N:1", targetName: "dim_supplier",            type: "dim",  businessName: "Suppliers",                description: "Vendor master merged with party table for legal name and address" },
    ],
    inventory: [
      // InventTrans must JOIN InventDim — InventDimId is mandatory (N:1, not standalone)
      { sourceTable: "InventTrans",              additionalSources: ["InventDim"],             relationship: "N:1", targetName: "fact_inventory_movements", type: "fact", businessName: "Inventory Movements",      description: "InventTrans (movement: qty, value, transaction type, cost amount) merged with InventDim (site, warehouse, batch, serial, location) via InventDimId. Without InventDim join, dimensions are unresolvable." },
      { sourceTable: "InventSum",                relationship: "1:1",                                           targetName: "fact_inventory_snapshot",  type: "fact", businessName: "Inventory Snapshot (On-Hand)", description: "InventSum — on-hand qty by item, dimension group (site, warehouse, batch). Kimball periodic snapshot fact. Join InventDim for full dimension resolution." },
      { sourceTable: "InventTable",              additionalSources: ["EcoResProduct","InventItemGroupItem"], relationship: "N:1", targetName: "dim_item", type: "dim", businessName: "Items / Products",       description: "InventTable (item, unit, model group) merged with EcoResProduct (product definition) and InventItemGroupItem (item group for GL mapping) via ItemId" },
      { sourceTable: "InventLocation",           relationship: "1:1",                                           targetName: "dim_warehouse",           type: "dim",  businessName: "Warehouses / Sites",       description: "InventLocation (warehouse: name, site, type) — conformed dimension for all inventory and procurement facts" },
    ],
    human_resources: [
      { sourceTable: "HcmWorker",                additionalSources: ["DirPersonName","HcmEmployment","HcmPosition"], relationship: "N:1", targetName: "dim_employee", type: "dim", businessName: "Employees",          description: "HcmWorker (worker RecId, type: Employee/Contractor) merged with DirPersonName (first/last name), HcmEmployment (start/end dates, employment type), HcmPosition (title, department) via Worker RecId" },
      { sourceTable: "HcmEmployment",            relationship: "1:N", additionalTargets: ["fact_employee_compensation","fact_position_history"], targetName: "fact_assignments", type: "fact", businessName: "Employee Assignments", description: "HcmEmployment → fact_assignments + HcmPositionWorkerAssignment (compensation level, pay rate) + position history for workforce analytics" },
      { sourceTable: "HcmAbsenceCode",           relationship: "1:1",                                           targetName: "dim_absence_code",         type: "dim",  businessName: "Leave / Absence Types",    description: "Absence type codes (HcmAbsenceCode) — sick leave, vacation, FMLA, parental, etc." },
      { sourceTable: "HcmDepartment",            relationship: "1:1",                                           targetName: "dim_department",           type: "dim",  businessName: "Departments",              description: "Department master (HcmDepartment) — cost center link, manager, org hierarchy level" },
    ],
  },

  // ── Oracle NetSuite ──────────────────────────────────────────────────────────
  // Sources: NetSuite SuiteAnalytics Connect schema (help.netsuite.com),
  // NetSuite Record Browser, Kimball Group.
  netsuite: {
    finance: [
      { sourceTable: "TRANSACTION",            additionalSources: ["TRANSACTION_LINES","ACCOUNTS"],          relationship: "N:1", targetName: "fact_gl_transactions", type: "fact", businessName: "GL Transactions", description: "TRANSACTION_LINES (account, debit/credit, department, class) merged with TRANSACTION header and ACCOUNTS (account type) via TRANSACTION_ID. Covers journals, invoices, bills, payments." },
      { sourceTable: "ACCOUNTS",               relationship: "1:1",                                           targetName: "dim_account",             type: "dim",  businessName: "GL Accounts",              description: "NetSuite chart of accounts — account number, type (Income/Expense/Asset), sub-type, parent account for hierarchy" },
      { sourceTable: "ACCOUNTING_PERIODS",     relationship: "1:1",                                           targetName: "dim_period",              type: "dim",  businessName: "Accounting Periods",       description: "Fiscal periods — period name, start/end date, year, quarter, fiscal year label" },
      { sourceTable: "CURRENCIES",             relationship: "1:1",                                           targetName: "dim_currency",            type: "dim",  businessName: "Currencies",               description: "Currency codes, symbols, and exchange rate metadata" },
    ],
    order_management: [
      { sourceTable: "TRANSACTION_LINES",      additionalSources: ["TRANSACTION","ITEMS"],                  relationship: "N:1", targetName: "fact_order_lines",       type: "fact", businessName: "Sales Order Lines",        description: "TRANSACTION_LINES (type=SalesOrder: item, qty, unit price, discount) merged with TRANSACTION (header: customer, date, status) and ITEMS (item type, category) via TRANSACTION_ID+LINE_ID" },
      { sourceTable: "TRANSACTION",            relationship: "1:N", additionalTargets: ["fact_sales_invoices","fact_order_fulfillments"], targetName: "fact_sales_orders", type: "fact", businessName: "Sales Orders", description: "Sales order TRANSACTION → fact_sales_orders (header) + fact_sales_invoices (linked Invoice records) + fact_order_fulfillments (ItemFulfillment records)" },
      { sourceTable: "CUSTOMERS",              additionalSources: ["CONTACTS","ADDRESSES"],                 relationship: "N:1", targetName: "dim_customer",            type: "dim",  businessName: "Customers",                description: "Customer record merged with primary CONTACTS and ADDRESSES (billing/shipping) via CUSTOMER_ID" },
      { sourceTable: "ITEMS",                  additionalSources: ["ITEM_LOCATIONS"],                       relationship: "N:1", targetName: "dim_product",             type: "dim",  businessName: "Products / Items",         description: "Item master merged with ITEM_LOCATIONS (location-level reorder point, preferred vendor, bin) via ITEM_ID" },
    ],
    accounts_payable: [
      { sourceTable: "TRANSACTION_LINES",      additionalSources: ["TRANSACTION","VENDORS"],                relationship: "N:1", targetName: "fact_ap_invoice_lines",  type: "fact", businessName: "AP Invoice Lines",         description: "Bill TRANSACTION_LINES (account, amount, category) merged with Bill TRANSACTION (header: vendor, due date, terms) and VENDORS for spend analysis. Filter TRANSACTION.TYPE = 'VendBill'." },
      { sourceTable: "VENDORS",                additionalSources: ["ADDRESSES"],                            relationship: "N:1", targetName: "dim_supplier",            type: "dim",  businessName: "Suppliers / Vendors",      description: "Vendor record merged with primary remittance ADDRESSES via VENDOR_ID" },
    ],
    procurement: [
      { sourceTable: "TRANSACTION_LINES",      additionalSources: ["TRANSACTION"],                         relationship: "N:1", targetName: "fact_po_lines",           type: "fact", businessName: "PO Lines",                 description: "Purchase order TRANSACTION_LINES (item, qty, rate) merged with PurchaseOrder TRANSACTION (header: vendor, ship-to, approval status) via TRANSACTION_ID. Filter TYPE = 'PurchOrd'." },
      { sourceTable: "VENDORS",                additionalSources: ["ADDRESSES"],                            relationship: "N:1", targetName: "dim_supplier",            type: "dim",  businessName: "Suppliers",                description: "Vendor master merged with remittance address" },
      { sourceTable: "ITEMS",                  relationship: "1:1",                                           targetName: "dim_item",                type: "dim",  businessName: "Items",                    description: "Purchaseable item master with preferred vendor, purchase description, and GL impact accounts" },
    ],
    inventory: [
      { sourceTable: "INVENTORY_BALANCE",      relationship: "1:1",                                           targetName: "fact_inventory_snapshot",  type: "fact", businessName: "Inventory Snapshot",       description: "INVENTORY_BALANCE — on-hand quantity and value by item and location (Kimball periodic snapshot fact)" },
      { sourceTable: "ITEMS",                  additionalSources: ["ITEM_LOCATIONS"],                       relationship: "N:1", targetName: "dim_item",                type: "dim",  businessName: "Items",                    description: "Item master merged with ITEM_LOCATIONS bin/reorder settings" },
      { sourceTable: "LOCATIONS",              relationship: "1:1",                                           targetName: "dim_warehouse",           type: "dim",  businessName: "Locations / Warehouses",   description: "Inventory location hierarchy — parent location, type (warehouse/bin), subsidiary" },
    ],
    human_resources: [
      { sourceTable: "EMPLOYEES",              additionalSources: ["CONTACTS","JOB_CODES"],                 relationship: "N:1", targetName: "dim_employee",            type: "dim",  businessName: "Employees",                description: "Employee record merged with CONTACTS (personal info) and JOB_CODES (title, pay type, department) via EMPLOYEE_ID" },
      { sourceTable: "DEPARTMENTS",            relationship: "1:1",                                           targetName: "dim_department",          type: "dim",  businessName: "Departments",              description: "Department master with parent department, budget owner, subsidiary" },
    ],
  },

  // ── Workday ──────────────────────────────────────────────────────────────────
  // Sources: Workday Report Designer field catalog, Workday PRISM Analytics schema,
  // Workday Community documentation, Kimball Group.
  workday: {
    finance: [
      { sourceTable: "Ledger_Account_Line",    additionalSources: ["Journal","Accounting_Period"],          relationship: "N:1", targetName: "fact_gl_transactions",    type: "fact", businessName: "GL Transactions",          description: "Ledger_Account_Line (account, debit/credit, spend category, cost center) merged with Journal (header: memo, source, status) and Accounting_Period via Journal Reference. Kimball grain: one account line." },
      { sourceTable: "Ledger_Account",         relationship: "1:1",                                           targetName: "dim_account",             type: "dim",  businessName: "Ledger Accounts",          description: "Workday Ledger_Account — account ID, account type (Asset/Liability/Revenue/Expense), summary level for roll-up" },
      { sourceTable: "Fiscal_Time_Period",     relationship: "1:1",                                           targetName: "dim_period",              type: "dim",  businessName: "Fiscal Periods",           description: "Workday Fiscal_Time_Period — period name, start/end date, fiscal year, quarter, company calendar" },
      { sourceTable: "Currency_Rate",          relationship: "1:1",                                           targetName: "dim_currency_rate",       type: "dim",  businessName: "Exchange Rates",           description: "Workday daily Currency_Rate — from/to currency pair, rate type, effective date" },
    ],
    accounts_payable: [
      { sourceTable: "Supplier_Invoice_Line",  additionalSources: ["Supplier_Invoice"],                    relationship: "N:1", targetName: "fact_ap_invoice_lines",  type: "fact", businessName: "AP Invoice Lines",         description: "Supplier_Invoice_Line (line amount, spend category, item description, tax) merged with Supplier_Invoice (header: supplier, invoice date, due date, approval status) via Invoice ID. Kimball AP fact." },
      { sourceTable: "Supplier",               additionalSources: ["Supplier_Contact"],                    relationship: "N:1", targetName: "dim_supplier",            type: "dim",  businessName: "Suppliers",                description: "Workday Supplier master merged with Supplier_Contact (primary AP contact, remit email) via Supplier ID" },
      { sourceTable: "Payment_Term",           relationship: "1:1",                                           targetName: "dim_payment_terms",       type: "dim",  businessName: "Payment Terms",            description: "Workday Payment_Term — net days, discount percent, discount days definitions" },
    ],
    procurement: [
      { sourceTable: "Purchase_Order_Line",    additionalSources: ["Purchase_Order"],                      relationship: "N:1", targetName: "fact_po_lines",           type: "fact", businessName: "PO Lines",                 description: "Purchase_Order_Line (spend category, qty, unit price, requester, deliver-to location) merged with Purchase_Order (header: supplier, order date, approval chain) via PO Number" },
      { sourceTable: "Supplier",               relationship: "1:1",                                           targetName: "dim_supplier",            type: "dim",  businessName: "Suppliers",                description: "Workday Supplier master for procurement dimension" },
      { sourceTable: "Spend_Category",         relationship: "1:1",                                           targetName: "dim_spend_category",      type: "dim",  businessName: "Spend Categories",         description: "Workday Spend_Category hierarchy — parent category, commodity code, GL account mapping for procurement analytics" },
    ],
    human_resources: [
      { sourceTable: "Worker",                 additionalSources: ["Worker_Profile","Employment_Data","Position"], relationship: "N:1", targetName: "dim_employee", type: "dim",  businessName: "Employees / Workers",      description: "Worker (worker ID, type) merged with Worker_Profile (legal name, preferred name, contact), Employment_Data (hire date, status, FTE), and Position (job profile, supervisory org) via Worker ID. Kimball conformed employee dimension." },
      { sourceTable: "Worker",                 relationship: "1:N", additionalTargets: ["fact_compensation_history","fact_employee_actions"], targetName: "fact_assignments", type: "fact", businessName: "Assignments / Workforce", description: "Worker → fact_assignments (current position + org) + fact_compensation_history (salary, stock, bonus changes via Compensation_History) + fact_employee_actions (hire, transfer, promote, terminate via Business_Process)" },
      { sourceTable: "Absence_Input",          relationship: "1:1",                                           targetName: "fact_absence",            type: "fact", businessName: "Absence / Leave",          description: "Workday Absence_Input — approved leave requests with worker, absence type, calendar days, hours, cost center allocation" },
      { sourceTable: "Cost_Center",            relationship: "1:1",                                           targetName: "dim_department",          type: "dim",  businessName: "Cost Centers / Depts",     description: "Workday Cost_Center — hierarchical org structure, manager, budget owner, company assignment" },
      // Workday Payroll: Payslip (header) + Payslip_Earning + Payslip_Deduction + Payslip_Tax (detail lines) N:1
      { sourceTable: "Payslip_Earning",        additionalSources: ["Payslip","Payslip_Deduction","Payslip_Tax"], relationship: "N:1", targetName: "fact_payroll",       type: "fact", businessName: "Payroll Detail (Earnings / Deductions / Tax)", description: "Workday Payroll grain: one row per payslip component. Payslip_Earning (base pay, OT, bonus) + Payslip_Deduction (benefits, garnishments) + Payslip_Tax (federal, state, local withholding) all merged with Payslip header (worker, period, net pay) via Payslip ID. Standard Kimball payroll accumulation fact." },
    ],
  },
};
